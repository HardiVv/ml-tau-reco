...
class TauJetDataset(Dataset):
    ...
    def process_file_data(self, data, filter_leptonic=True): # see on põhi funkt
        ...
        #for training, we might want to filter out leptonic taus
        if filter_leptonic:
            jet_inds = [ijet for ijet in range(len(jet_features)) if gen_tau_decaymode[ijet]!=16]
        else:
        #for inference, we need to keep everything
            jet_inds = list(range(len(jet_features)))
